...
- **Automático (en vivo):** un proceso programado en GitHub baja el Excel de SharePoint cada hora y regenera `data/data.json` solo. El tablero muestra "Actualizado dd mmm hh:mm" arriba a la derecha. **Guía completa paso a paso en [`docs/CONEXION_EN_VIVO.md`](docs/CONEXION_EN_VIVO.md).**

> Importante: una página web no puede leer un SharePoint privado directamente desde el navegador (lo bloquea Microsoft por seguridad). Por eso la actualización corre del lado del servidor en GitHub Actions, de forma segura. El navegador solo lee el `data.json` resultante.
